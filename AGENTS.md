...
This document defines the rules, context, and standards for AI coding agents
when contributing to the `multifold` project (Rust Edition). Read this
carefully before generating code.
...
* Before committing, ensure the `check` job (Quality Check) in
  `.github/workflows/ci.yml` would pass for the changed files.
* Before pushing or opening a PR, ensure all CI jobs pass (`check`, `test`).
